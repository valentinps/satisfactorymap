...
/// each op's re-parse. The prior state is dropped before each re-parse so
/// peak memory stays ~one parsed save (load-bearing for 4GB-limited wasm on
/// 600k-object saves).
pub fn rebuild(
    ...
    for (i, op) in ops.iter().enumerate() {
        store = step_owned(store, op, tables)?;
...
/// step(), but consumes the store: the op is planned against the parsed
/// structures, which are then dropped, and the plan is applied IN PLACE on
/// the store's own body buffer before the re-parse. Peak memory stays ~one
