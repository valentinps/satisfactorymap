...
const INVENTORY_PROPERTY_NAMES: [&[u8]; 9] = [
    b"mInventory",
    b"mInputInventory",
    b"mFuelInventory",
    b"mOutputInventory",
...
const VEHICLE_INVENTORY_COMPONENT_SUFFIXES: [&[u8]; 2] =
    [b".StorageInventory", b".FuelInventory"];

fn find_prop<'a>(props: &'a PropList, data: &[u8], name: &[u8]) -> Option<&'a PropertyValue> {
    props
        .props
        .iter()
...
fn stack_item<'a>(pl: &PropList, data: &'a [u8]) -> Option<(&'a [u8], i64)> {
...
#[pyfunction]
pub fn collect_item_location_index(py: Python<'_>, save: PyRef<'_, ParsedSavePy>) -> PyResult<PyObject> {
    ...
    let index = py.allow_threads(|| {
        ...
        let object_at =
            |(li, oi): (usize, usize)| -> &Object { &store.levels[li].objects[oi] };
        ...
        let rows: Vec<(Vec<u8>, Vec<(Vec<u8>, i64)>)> = index
            .order
            .into_iter()
            .map(|(short, entries)| {
                (
    ...
    build_item_index_dict(py, out.clone(), index)?;
    Ok(out.into_any().unbind())
}

fn build_item_index_dict(
    py: Python<'_>,
    out: Bound<'_, PyDict>,
    index: Vec<(Vec<u8>, Vec<(Vec<u8>, i64)>)>,
) -> PyResult<()> {
...
fn vector_prop(pl: &PropList, data: &[u8], name: &[u8]) -> Option<[f64; 3]> {
...
/// length check. Returns list[(instanceName, typePath, flatPoints)].
#[pyfunction]
pub fn collect_spline_polylines(
    ...
    let results: Vec<(String, String, Vec<f64>)> = py.allow_threads(|| {
        let data: &[u8] = &store.data;
        let wanted: std::collections::HashSet<&[u8]> =
            type_paths.iter().map(|s| s.as_bytes()).collect();
                    ...
                    Some(PropertyValue::Array(ArrayValue::Structs(points))) => {
                        for point in points {
                            if let Some(location) = vector_prop(point, data, b"Location") {
                                let arrive =
                                    vector_prop(point, data, b"ArriveTangent").unwrap_or(zero);
                                let leave =
                                    vector_prop(point, data, b"LeaveTangent").unwrap_or(zero);
